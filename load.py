...
# -------------------------------------------------------------------------
# COMMODITY MAPPING & FORMATTING (Optimized)
# -------------------------------------------------------------------------
_C_RAW = {
...
def clean_station_name(name):
    """Uses robust regex to safely strip formatting regardless of colons, spaces or exact spelling"""
    if not name: return ""
...
def restore_active_project():
    ...
    if b_id:
        ...
        try:
            active_project["market_id"] = int(config.get_str("RCC_ActiveMarketId") or "0")
        except ValueError:
            active_project["market_id"] = 0

        log_info(f"Restored active project from cache: {active_project['name']} ({b_id})")
...
def unlink_project():
    ...
    save_active_project()
    if hud_instance:
        hud_instance.update_system(current_system['name'], force_show=True)
...
def set_current_system(name, addr, pos):
    ...
    if changed:
        log_info(f"System updated to {name} ({current_system['address']})")
        trigger_system_update(name)

def set_last_docked(name, m_id):
    global last_docked_station
    if not name: return
    try:
        m_id = int(m_id) if m_id else 0
    except ValueError:
        m_id = 0
    ...
    log_debug(f"Saved persistent station: {name} (Market: {m_id})")
...
def resolve_market_id(target_name):
    ...
    if monitor and getattr(monitor, 'state', None):
        ...
        if m_id and str(m_id) != "0" and s_name:
            if s_name == target_lower or s_name in target_lower or target_lower in s_name:
                return m_id
    ...
    if last_docked_station["market_id"] and str(last_docked_station["market_id"]) != "0":
        ...
        if ld_name and (ld_name == target_lower or ld_name in target_lower or target_lower in ld_name):
            return last_docked_station["market_id"]

    for s_name, m_id in system_stations_cache.items():
        s_name_lower = clean_station_name(s_name).lower()
        if m_id and str(m_id) != "0":
            if s_name_lower == target_lower or s_name_lower in target_lower or target_lower in s_name_lower:
                return m_id

    if monitor and getattr(monitor, 'state', None):
        m_id = monitor.state.get("MarketID")
        if m_id and str(m_id) != "0": return m_id
...
def blend_color(hex_color, opacity_factor):
    if opacity_factor >= 1.0: return hex_color
    if opacity_factor <= 0.0: return "#000000"
    hex_color = hex_color.lstrip('#')
    r, g, b = tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))
    r = int(r * opacity_factor)
    g = int(g * opacity_factor)
    b = int(b * opacity_factor)
...
def load_build_data(plugin_dir):
    ...
    try:
        ...
        log_error(f"Error reading JSON: {e}")

    try:
        for filename in os.listdir(plugin_dir):
            if filename.lower() == 'colonization-costs2.json':
                alt_path = os.path.join(plugin_dir, filename)
        ...
        log_error(f"Error in fallback loader: {e}")
...
def get_colonial_buffs(body):
    ...
    if body.get('volcanismType') or signals.get('Geological', 0) > 0:
        buffs.update(["Extraction"])
...
class OverlayHUD:
    def __init__(self):
        ...
        self.lbl_jump.pack(pady=(2, 5), padx=15)
    ...
    def _apply_settings(self):
        try: bg_opacity = float(config.get_str("RCC_HUDBgOpacity") or "1.0")
        except ValueError: bg_opacity = 1.0

        try: txt_opacity = float(config.get_str("RCC_HUDTextOpacity") or "1.0")
        except ValueError: txt_opacity = 1.0
        ...
        c_name = config.get_str("RCC_HUDColor") or "Orange"
        c_hex = color_map.get(c_name, "#ffcc00")
        ...
        always_on_top = config.get_str("RCC_HUDAlwaysOnTop") != "0" # Default true
        ...
        bg_mode = config.get_str("RCC_HUDBgMode") or "Solid"
        if bg_mode.startswith("Transparent"):
            try: self.root.attributes("-transparentcolor", "black")
            except: pass
        ...
        self.lbl_system.config(font=("Arial", int(14 * scale), "bold"), fg=blend_color("#ff8c00", txt_opacity))
        self.lbl_project.config(font=("Arial", int(10 * scale), "italic"), fg=blend_color("#00ff00", txt_opacity))
        self.lbl_jump.config(font=("Arial", int(9 * scale), "italic"), fg=blend_color("#aaaaaa", txt_opacity))
        self.lbl_signal.config(font=("Arial", int(11 * scale)), fg=blend_color("#00ffff", txt_opacity))
        self.lbl_status.config(font=("Arial", int(9 * scale)), fg=blend_color("#aaaaaa", txt_opacity))

        self.lbl_progress.config(
            font=("Consolas", int(9 * scale)),
            fg=blend_color(c_hex, txt_opacity),
    ...
    def _update_system(self, system_name, force_show):
        ...
        if config.get_str("RCC_HUDShowAllProjects") == "1":
            new_proj = "System Projects:"
        else:
    ...
    def _update_progress(self, projects_data, force_show):
        ...
        for proj in projects_data:
            ...
            for cat in sorted_cats:
                ...
                for name, amount in grouped[cat]:
...
class ColonialReportMenu:
    def __init__(self, parent_frame):
        ...
        if not system_colonial_report:
            self.tree.insert("", tk.END, values=("No Data", "No prime bodies or EDSM is still syncing.", ""))
        else:
            for item in system_colonial_report:
                self.tree.insert("", tk.END, values=(item['name'], item['tag'], item['buffs']))
...
class LinkProjectMenu:
    ...
    def fetch_planned_sites(self):
        ...
        try:
            ...
            log_debug(f"Fetching sites from: {url}")
            resp = session.get(url, timeout=10)
            if resp.status_code == 200:
                sites = resp.json()
                planned_sites = [s for s in sites if s.get("status") in ["plan", "build"]]
                log_debug(f"Found {len(planned_sites)} plannable/active sites.")
    ...
    def on_select(self, event=None):
        ...
        if db_market_id and str(db_market_id) != "0":
            ...
            detected_id = resolve_market_id(site_data.get("name", ""))
            self.market_id_var.set(str(detected_id))
    ...
    def start_project(self):
        ...
        if not body_val:
            ...
            if body_num is not None: body_val = str(body_num)
            else: body_val = ""
    ...
    def __init__(self, parent_frame):
        ...
        try:
            ...
            if monitor and getattr(monitor, 'state', None):
                ...
                if not raw_body:
                    raw_body = monitor.state.get("Body")

                target_body = str(raw_body) if raw_body else ""
                sys_name = current_system.get("name", "")

                if sys_name and target_body.startswith(sys_name):
                    target_body = target_body[len(sys_name):].strip()
                elif target_body == sys_name:
                    target_body = ""
            ...
            # Fallback to persistent memory if we are docked but monitor state is missing
            if not current_station and last_docked_station.get("name"):
                current_station = str(last_docked_station["name"])
            if not current_market_id and last_docked_station.get("market_id"):
                current_market_id = last_docked_station["market_id"]
            ...
            if station_type and station_type in keys:
                self.type_combo.current(keys.index(station_type))
            elif keys:
            ...
            # Explicitly assigning the window as the master for these variables prevents Linux UI lockups
            ...
            # If the UI still fails, it will render the traceback text directly to the window instead of remaining blank
            import traceback
            err_msg = traceback.format_exc()
            log_error(f"UI Crash in NewColonyMenu: {err_msg}")

            tk.Label(self.window, text="UI Initialization Failed", fg="#ff4444", font=("Arial", 11, "bold")).pack(pady=5)
            text_widget = tk.Text(self.window, height=18, width=45, bg="#1e1e1e", fg="#ffdddd")
...
def read_market_json():
    ...
    try:
        ...
        if jdir:
            paths_to_check.append(os.path.join(jdir, 'Market.json'))
            paths_to_check.append(os.path.abspath(os.path.join(jdir, 'Market.json')))
    except: pass
    ...
    if os.name == 'nt':
        ...
        paths_to_check.append(os.path.expanduser(r"~\Saved Games\Frontier Developments\Elite Dangerous\Market.json"))
    ...
    for p in paths_to_check:
        ...
        checked.add(p)
        log_debug(f"Checking for Market.json at: {p}")
        if os.path.exists(p):
            log_debug(f"SUCCESS: Found Market.json at {p}")
            try:
...
def parse_market_data(market_data):
    ...
    try:
        ...
        log_debug(f"parse_market_data received payload. Extracted MarketID: {m_id}")

        if not m_id:
            log_error("MarketID was missing or null in market payload.")
            return
        ...
        log_debug(f"Scanning {len(items)} commodities in payload...")

        debug_items = []
        for item in items:
            demand = item.get("demand")
            if demand is None: demand = item.get("Demand", 0)

            name = item.get("name") or item.get("Name", "")
            debug_items.append(f"{name}:{demand}")

            if demand > 0:
                name = name.lower().strip()
                if name.startswith('$'):
                    name = name.replace('$', '').replace('_name;', '').strip()
                live_demands[name] = demand

        log_debug(f"Market Dump ({m_id}): " + ", ".join(debug_items[:20]) + ("..." if len(debug_items) > 20 else ""))

        if live_demands:
            latest_market_data["market_id"] = m_id
            latest_market_data["demands"] = live_demands
            log_info(f"Captured {len(live_demands)} active demands for Market ID: {m_id}")
            ...
            log_debug(f"Market ID {m_id} contains ZERO commodities with demand > 0.")

    except Exception as e:
        log_error(f"Live Market Parse Error: {e}")
        ...
        if monitor and getattr(monitor, 'state', None):
            if current_system["name"] == "Unknown":
                ...
                if sys_name and sys_name != "Unknown":
                    set_current_system(
                        sys_name,
                        monitor.state.get("SystemAddress", 0),
                        monitor.state.get("StarPos", [0.0, 0.0, 0.0])
                    )
            ...
            if m_id and s_name and str(m_id) != "0":
                try:
                    m_id_int = int(m_id)
                    if last_docked_station["market_id"] != m_id_int:
                        set_last_docked(s_name, m_id_int)
            ...
            if resp.status_code == 200:
                ...
                for s in build_sites:
                    ...
                    if p_resp.status_code == 200:
                        ...
                        projects_render_data.append({
                            "title": f"{display_type}: {clean_name}",
                            "demands": comms
                        })

                if hud_instance:
                    hud_instance.update_progress(projects_render_data)
                return
        except Exception:
            pass
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if comms is not None:
                    active_project["progress_data"] = comms
                    if hud_instance:
                        hud_instance.update_progress([{"title": None, "demands": comms}])
        except Exception:
            pass
...
def sync_live_market_to_server(build_id, live_demands):
    ...
    try:
        url = f"{RCC_API_BASE}/api/project/{urllib.parse.quote(build_id)}"
        resp = session.post(url, json=payload, headers=headers, timeout=10)
        if resp.status_code in [200, 201, 204]:
            log_debug(f"Live Market Sync Successful for {build_id}")
        else:
            log_error(f"Live Market Sync Failed {resp.status_code}: {resp.text}")
    except Exception as e:
...
api_key_var = None
cmdr_name_var = None
hud_bg_opacity_var = None
hud_txt_opacity_var = None
hud_scale_var = None
hud_color_var = None
hud_hide_var = None
...
def plugin_prefs(parent, cmdr, is_beta):
    global api_key_var, cmdr_name_var, hud_bg_opacity_var, hud_txt_opacity_var, hud_scale_var, hud_color_var, hud_hide_var, hud_columns_var, hud_align_var, hud_jump_var, hud_journal_path_var, hud_all_projects_var, hud_hide_tools_var, hud_hide_actions_var, hud_always_on_top_var
    ...
    val_jump = config.get_str("RCC_HUDShowJump")
    hud_jump_var = tk.IntVar(value=0 if val_jump == "0" else 1)

    val_all_proj = config.get_str("RCC_HUDShowAllProjects")
    hud_all_projects_var = tk.IntVar(value=1 if val_all_proj == "1" else 0)

    val_hide_tools = config.get_str("RCC_HideTools")
    hud_hide_tools_var = tk.IntVar(value=1 if val_hide_tools == "1" else 0)

    val_hide_actions = config.get_str("RCC_HideActions")
    hud_hide_actions_var = tk.IntVar(value=1 if val_hide_actions == "1" else 0)

    val_always_on_top = config.get_str("RCC_HUDAlwaysOnTop")
    hud_always_on_top_var = tk.IntVar(value=0 if val_always_on_top == "0" else 1)

    try: bg_op_val = float(config.get_str("RCC_HUDBgOpacity") or "1.0")
    except ValueError: bg_op_val = 1.0
    hud_bg_opacity_var = tk.DoubleVar(value=bg_op_val)

    try: txt_op_val = float(config.get_str("RCC_HUDTextOpacity") or "1.0")
    except ValueError: txt_op_val = 1.0
    hud_txt_opacity_var = tk.DoubleVar(value=txt_op_val)
    ...
    tk.Label(frame, text="HUD Layout Columns:").grid(row=3, column=0, sticky=tk.W, padx=10, pady=5)
    ttk.Combobox(frame, textvariable=hud_columns_var, values=["Auto", "1 Column", "2 Columns"], state="readonly").grid(row=3, column=1, sticky=tk.W, padx=10)

    tk.Label(frame, text="HUD Background Opacity:").grid(row=4, column=0, sticky=tk.W, padx=10, pady=5)
    tk.Scale(frame, variable=hud_bg_opacity_var, from_=0.0, to=1.0, resolution=0.05, orient=tk.HORIZONTAL).grid(row=4, column=1, sticky=tk.W, padx=10)

    tk.Label(frame, text="HUD Text Opacity:").grid(row=5, column=0, sticky=tk.W, padx=10, pady=5)
    tk.Scale(frame, variable=hud_txt_opacity_var, from_=0.0, to=1.0, resolution=0.05, orient=tk.HORIZONTAL).grid(row=5, column=1, sticky=tk.W, padx=10)
...
def prefs_changed(cmdr, is_beta):
    ...
    if hud_bg_opacity_var: config.set("RCC_HUDBgOpacity", str(hud_bg_opacity_var.get()))
    if hud_txt_opacity_var: config.set("RCC_HUDTextOpacity", str(hud_txt_opacity_var.get()))
    ...
    # Robust visibility re-stacking to preserve order
...
def plugin_start3(plugin_dir):
    global hud_instance, tools_frame_ref, actions_frame_ref
...
def plugin_app(parent):
    global tools_frame_ref, actions_frame_ref, overlay_toggle_var
    ...
    if config.get_str("RCC_HideActions") != "1":
        actions_frame_ref.pack(fill=tk.X, padx=5, pady=5)
    ...
    if config.get_str("RCC_HideTools") != "1":
        tools_frame_ref.pack(fill=tk.X, padx=5, pady=5)
...
def create_raven_project_api():
    ...
    exact_name = None
    if market_id != 0:
        if monitor and getattr(monitor, 'state', None) and monitor.state.get("MarketID") == market_id:
            exact_name = monitor.state.get("StationName")
        elif last_docked_station["market_id"] == market_id:
            exact_name = last_docked_station["name"]
        else:
            for s_name, m_id in system_stations_cache.items():
                if m_id == market_id:
                    exact_name = s_name
    ...
    if exact_name:
        ...
        if clean_exact != active_project["name"]:
            log_info(f"Auto-renamed project to '{clean_exact}' based on Market ID match.")
            active_project["name"] = clean_exact
    ...
    if build_id:
        ...
        link_url = f"{RCC_API_BASE}/api/project/{urllib.parse.quote(build_id)}/link/{urllib.parse.quote(cmdr_name)}"
        log_debug(f"PUT to {link_url}")

        try:
            response = session.put(link_url, headers=headers, timeout=10)
            if response.status_code in [200, 201, 204]:
                save_active_project()

                if latest_market_data["market_id"] == active_project["market_id"] and latest_market_data["demands"]:
                    if not active_project.get("force_bypass"):
                ...
                if active_project["auto_open_browser"]:
                    webbrowser.open(f"{RCC_UX_BASE}/#build={urllib.parse.quote(build_id)}")
                return
            else:
                err_text = response.text.strip()
                try:
                    err_json = response.json()
                    if "detail" in err_json: err_text = str(err_json["detail"])
                    elif "errors" in err_json and err_json["errors"]: err_text = str(err_json["errors"])
                except Exception: pass
    ...
    try:
        ...
        pos_arr = current_system.get("pos", [0.0, 0.0, 0.0])
        btype = active_project["build_type"]

        cargo_dict = BUILD_CARGO_MAP.get(str(btype).lower(), {}).copy()

        log_debug(f"Checking live data match: Project MarketID={market_id}, Latest Captured={latest_market_data['market_id']}")
        has_live = False
        if market_id != 0 and latest_market_data["market_id"] == market_id and latest_market_data["demands"]:
            has_live = True
        ...
        if active_project.get("force_bypass") and active_project.get("manual_cargo_dict"):
            ...
            log_info("User manually entered commodity amounts. Bypassing Live Market check.")

        if has_live:
            if not active_project.get("force_bypass"):
                cargo_dict = latest_market_data["demands"].copy()
                log_info(f"Using exact LIVE market data for variance. Items: {len(cargo_dict)}")
        else:
            trigger_error_popup(
                "Missing Live Market Data",
                "To capture the exact commodity requirements (due to in-game variance), you MUST physically dock and view the market board in-game BEFORE linking a new planned colony.\n\nPlease close this, open the market board, and try again! (Or use the bypass checkbox if servers are down)."
            )
        ...
        if raw_body.isdigit() or (raw_body.startswith('-') and raw_body[1:].isdigit()):
            b_num = int(raw_body)
            body_name_str = f"{current_system['name']} {b_num}"
        else:
            b_num = -1
            if not raw_body or raw_body.lower() in ["none", "null"]:
                body_name_str = current_system['name']
            elif not raw_body.lower().startswith(current_system["name"].lower()):
        ...
        payload = {
            "buildType": str(btype),
            "buildName": str(active_project["name"]),
            "marketId": int(active_project.get("market_id", 0)),
            "systemAddress": int(current_system["address"]),
            "systemName": str(current_system["name"]),
            "starPos": [float(p) for p in pos_arr],
            "architectName": cmdr_name,
            "maxNeed": int(max_need),
            "commodities": cargo_dict,
            "bodyName": body_name_str,
            "bodyNum": b_num if b_num >= 0 else 0,
            "isPrimaryPort": False if site_id else True
        }

        if site_id:
            payload["systemSiteId"] = str(site_id)

        headers = {"rcc-key": api_key, "rcc-cmdr": cmdr_name, "Content-Type": "application/json"}

        log_debug(f"PUT to /api/project/ Payload:\n{json.dumps(payload, indent=2)}")
        ...
        if response.status_code in [200, 201]:
            ...
            if build_id and site_id:
                ...
                update_url = f"{RCC_API_BASE}/api/v2/system/{urllib.parse.quote(current_system['name'])}/sites"
                session.put(update_url, json=update_payload, headers=headers, timeout=10)
            ...
            try:
                ...
                elif "title" in err_json: err_text = str(err_json["title"])
            except Exception: pass
...
def publish_current_ship(cmdr_name, ship_name, ship_type, cargo_dict):
    ...
    payload = {"cmdr": get_cmdr_name(), "name": ship_name, "type": ship_type, "maxCargo": max_cargo, "cargo": cargo_dict}
    headers = {"rcc-key": api_key, "Content-Type": "application/json"}
    try: session.post(f"{RCC_API_BASE}/api/cmdr/currentShip", json=payload, headers=headers, timeout=10)
...
def contribute_to_project(build_id, cmdr_name, cargo_diff):
    ...
    api_key = config.get_str("RCC_ApiKey")
    headers = {"Content-Type": "application/json", "rcc-key": api_key}
    try:
        url = f"{RCC_API_BASE}/api/project/{urllib.parse.quote(build_id)}/contribute/{urllib.parse.quote(get_cmdr_name())}"
        resp = session.post(url, json=cargo_diff, headers=headers, timeout=10)
        if resp.status_code in [200, 201, 204]:
            log_debug(f"Contribution successful: {cargo_diff}")
            threading.Thread(target=fetch_project_progress, daemon=True).start()
        else:
            log_error(f"Contribution Failed {resp.status_code}: {resp.text}")
    except Exception as e: log_error(f"Contribution Error: {e}")

def publish_fleet_carrier(cmdr_name, market_id, name, callsign):
    api_key = config.get_str("RCC_ApiKey")
    if not api_key: return
    payload = {"marketId": market_id, "name": callsign, "displayName": name, "cargo": None}
    headers = {"rcc-key": api_key, "rcc-cmdr": get_cmdr_name(), "Content-Type": "application/json"}
    try: session.put(f"{RCC_API_BASE}/api/fc/{market_id}", json=payload, headers=headers, timeout=10)
    except Exception as e: log_error(f"FC Sync Error: {e}")

def update_sys_bodies(address, bods):
    api_key = config.get_str("RCC_ApiKey")
    if not api_key or not bods: return
    headers = {"rcc-key": api_key, "Content-Type": "application/json"}
    try: session.put(f"{RCC_API_BASE}/api/v2/system/{address}/bodies", json=bods, headers=headers, timeout=15)
...
def fetch_edsm_data(system_name):
    ...
    system_colonial_report = []
    try:
        url_stations = f"https://www.edsm.net/api-system-v1/stations?systemName={urllib.parse.quote(system_name)}"
        resp_stations = session.get(url_stations, timeout=10)
        ...
        if resp_stations.status_code == 200:
            ...
            if isinstance(data_stations, dict) and 'stations' in data_stations:
                for st in data_stations.get('stations', []):
                    if 'name' in st and 'marketId' in st:
                        system_stations_cache[st['name']] = st['marketId']

        url_bodies = f"https://www.edsm.net/api-system-v1/bodies?systemName={urllib.parse.quote(system_name)}"
        resp_bodies = session.get(url_bodies, timeout=10)
        ...
        if resp_bodies.status_code == 200:
            ...
            for body in data_bodies.get('bodies', []):
                ...
                if body.get('terraformingState') in ['Terraformable', 'Terraforming', 'Terraformed']:
                    is_interesting = True
                ...
                if is_interesting:
                    interesting_count += 1
                    buffs = get_colonial_buffs(body)
                    tag = subType + (f" ({', '.join(reasons)})" if reasons else "")
                    system_colonial_report.append({"name": name or "Main Star", "tag": tag, "buffs": buffs})

            if hud_instance:
                hud_msg = f"Prime Targets: {interesting_count} | Bio: {bio_count} | Geo: {geo_count}"
                hud_instance.update_signals(hud_msg, force_show=True)
    except Exception as e:
        logger.error(f"[{plugin_name}] EDSM Parse Error: {e}")

def map_scan_to_bod(entry):
    return {
...
def journal_entry(cmdr, is_beta, system, station, entry, state):
    global current_system, active_project, system_scans_cache, last_docked_station, latest_market_data
    event = entry.get('event')

    try:
        if state and state.get('SystemName') and current_system['name'] != state.get('SystemName'):
            set_current_system(
                state.get('SystemName'),
                state.get('SystemAddress', 0),
                state.get('StarPos', [0.0, 0.0, 0.0])
            )
        ...
        if event == 'ColonisationConstructionDepot':
            ...
            if m_id:
                ...
                if live_demands:
                    latest_market_data["market_id"] = m_id
                    latest_market_data["demands"] = live_demands
                    log_info(f"Captured {len(live_demands)} active demands from ColonisationConstructionDepot event for Market ID: {m_id}")
            ...
            set_current_system(
                entry.get('StarSystem', 'Unknown System'),
                entry.get('SystemAddress', 0),
                entry.get('StarPos', [0.0, 0.0, 0.0])
            )
            ...
            if entry.get('Docked'):
                ...
                if active_project['is_active']:
                    c_docked = clean_station_name(s_name).lower()
                    c_target = clean_station_name(active_project['name']).lower()
                    if c_docked == c_target or c_docked in c_target or c_target in c_docked:
                        if active_project['market_id'] != m_id:
                            ...
                            save_active_project()
                            log_info(f"Auto-bound active project Market ID to {m_id} on Location event.")
            ...
            if config.get_str("RCC_HUDShowJump") != "0":
                ...
                jump_info = f"Pop: {pop:,} | {alleg} | {econ} | {sec}"
                if hud_instance:
                    hud_instance.safe_execute(lambda: hud_instance.jump_var.set(jump_info))
                    hud_instance.show_hud()
            else:
                if hud_instance:
                    hud_instance.safe_execute(lambda: hud_instance.jump_var.set(""))
            ...
            if body_id is not None: system_scans_cache[body_id] = map_scan_to_bod(entry)

        elif event == 'FSSAllBodiesFound':
            system_address = entry.get('SystemAddress', current_system['address'])
            if system_scans_cache:
                bods = list(system_scans_cache.values())
                threading.Thread(target=update_sys_bodies, args=(system_address, bods), daemon=True).start()
            ...
            if 'Inventory' in entry:
                for item in entry['Inventory']: cargo_dict[item['Name']] = item['Count']
            ...
            if active_project['is_active'] and active_project.get('build_id'):
                ...
                if m_id and str(m_id) == str(active_project.get('market_id')):
                    ...
                    if commodity.startswith('$'):
                        commodity = commodity.replace('$', '').replace('_name;', '').strip()
                    count = entry.get('Count', 0)
                    diff = {commodity: count}
                    threading.Thread(target=contribute_to_project, args=(active_project['build_id'], get_cmdr_name(), diff), daemon=True).start()

        elif event in ['CarrierJump', 'CarrierBuy']:
            market_id = entry.get('MarketID')
            name = entry.get('CarrierName', 'Unknown Carrier')
            callsign = entry.get('Callsign', 'XXX-XXX')
            threading.Thread(target=publish_fleet_carrier, args=(get_cmdr_name(), market_id, name, callsign), daemon=True).start()
            ...
            if active_project['is_active']:
                ...
                if c_docked == c_target or c_docked in c_target or c_target in c_docked:
                    if active_project['market_id'] != m_id:
                        active_project['market_id'] = m_id
                        save_active_project()
                        log_info(f"Auto-bound active project Market ID to {m_id} on Docked event.")
        ...
        log_error(f"Critical Journal Error processing event {event}: {e}")
